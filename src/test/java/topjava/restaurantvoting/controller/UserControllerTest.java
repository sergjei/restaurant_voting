...
class UserControllerTest extends AbstractControllerTest {
    ...
    void update() throws Exception {
        ...
        perform(MockMvcRequestBuilders.put(CURRENT_URL).contentType(MediaType.APPLICATION_JSON)
                .content(json))
                .andDo(print())
                .andExpect(status().isNoContent());
        USER_MATCHER.assertMatch(userRepository.findByEmailIgnoreCase(updated.getEmail()).orElseThrow(), updated);
    }
    ...
    void getVotes() throws Exception {
        ...
        Assert.isTrue(actual.size() == 1, "Must be only one vote");
